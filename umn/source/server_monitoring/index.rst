:original_name: ces_01_0120.html

.. _ces_01_0120:

Server Monitoring
=================

-  :ref:`Introduction to Server Monitoring <en-us_topic_0078721772>`
-  :ref:`Agent Installation and Configuration <ces_01_0027>`
-  :ref:`Installing and Configuring the Agent on a Linux ECS <ces_01_0004>`
-  :ref:`Installing and Configuring the Agent on a Windows ECS <ces_01_0005>`
-  :ref:`Installing the Agents in Batches on Linux ECSs <ces_01_0034>`
-  :ref:`Managing the Agent <ces_01_0035>`
-  :ref:`Viewing Server Monitoring Metrics <en-us_topic_0079332017>`
-  :ref:`Creating an Alarm Rule to Monitor a Server <ces_01_0042>`

.. toctree::
   :maxdepth: 1
   :hidden: 

   introduction_to_server_monitoring
   agent_installation_and_configuration
   installing_and_configuring_the_agent_on_a_linux_ecs/index
   installing_and_configuring_the_agent_on_a_windows_ecs/index
   installing_the_agents_in_batches_on_linux_ecss
   managing_the_agent/index
   viewing_server_monitoring_metrics
   creating_an_alarm_rule_to_monitor_a_server
